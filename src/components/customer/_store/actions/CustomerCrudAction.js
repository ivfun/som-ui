...
} from './CustomerCrudAction.types'

import CustomerServiceCrud from '../../services/Customer.service.crud';
import {customerListing} from "./CustomerScreenAction";
import {firstPage} from "../../../pagination/_store/actions/PaginationAction";

const fetchData = ()=>( CustomerServiceCrud.findAll());
...
const removeData = (id)=>( CustomerServiceCrud.remove(id));
...
const saveData = (data)=>(data && data.id && data.id !== '' ? CustomerServiceCrud.update(data):CustomerServiceCrud.create(data));
...
export function fetchDataAsync() {
    return function (dispatch) {
        ...
        return fetchData().then(
            data=>{
                dispatch(fetchDataSuccess(data.customers ?data.customers:[]));
                dispatch(firstPage());
            },
