...
} from './SolutionCrudAction.types'

import SolutionServiceCrud from '../../services/Solution.service.crud';
import {solutionListing} from "./SolutionScreenAction";
import {firstPage} from "../../../pagination/_store/actions/PaginationAction";

const fetchData = ()=>( SolutionServiceCrud.findAll());
...
const removeData = (id)=>( SolutionServiceCrud.remove(id));
...
const saveData = (data)=>(data && data.id && data.id !== '' ? SolutionServiceCrud.update(data):SolutionServiceCrud.create(data));
...
export function fetchDataAsync() {
    return function (dispatch) {
        ...
        return fetchData().then(
            data=>{
                dispatch(fetchDataSuccess(data.solutions ?data.solutions:[]));
                dispatch(firstPage());
            },
